...
public class InvertedIndexing {
    private static final Log LOG = LogFactory.getLog(InvertedIndexing.class);
    ...
    public static boolean indexExists() {
        if (new File("inverted_index.indx").isFile()) {
            return true;
        }
    ...
    public static void getIndexMap(int mode) throws IOException, InterruptedException {
        ...
        if (mode == 1) {
            ii = "inverted_index.indx";
        } else if (indexExists()) {
            pr = rt.exec("hadoop fs -get " + outPath + "/part-r-00000 temp.indx");
            pr.waitFor();
            ii = "temp.indx";
        } else {
            pr = rt.exec("hadoop fs -get " + outPath + "/part-r-00000 inverted_index.indx");
            pr.waitFor();
            ii = "inverted_index.indx";
        }
    ...
    public static void outputMap() throws IOException {
        ...
        new File("inverted_index.indx").delete();
        File file = new File("inverted_index.indx");
